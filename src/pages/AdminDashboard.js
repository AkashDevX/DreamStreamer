...
const AdminDashboard = ({ signOut }) => {
  ...
  const handleDeleteAlbum = async (albumId) => {
    ...
    try {
      await axios.delete(`https://puas37n2r2.execute-api.us-east-1.amazonaws.com/Development/dreamstreamer-albums/${albumId}`);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const albumArtResponse = await axios.post(
        'https://puas37n2r2.execute-api.us-east-1.amazonaws.com/Development/generating-s3links',
        {
      ...
      await axios.post('https://puas37n2r2.execute-api.us-east-1.amazonaws.com/Development/dreamstreamer-albums', albumMetadata);
  ...
  const handleUpdateAlbum = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://puas37n2r2.execute-api.us-east-1.amazonaws.com/Development/dreamstreamer-albums/${selectedAlbum.albumId}`,
        updatedAlbum
      );
  ...
  const requestAnalyticsReport = async () => {
    try {
        const response = await axios.post('https://puas37n2r2.execute-api.us-east-1.amazonaws.com/Development/sending-report');
...
<aside className="w-1/6 bg-gray-900 p-6 flex flex-col justify-between shadow-lg">
    <div>
        ...
        <nav>
            <ul className="space-y-4">
                {[
                    { name: 'Home', href: '#homenav', icon: <HomeIcon className="h-6 w-6 text-blue-400" /> }, // All icons in blue
                    { name: 'Explore', href: '#mangealbumsnav', icon: <MagnifyingGlassIcon className="h-6 w-6 text-blue-400" /> },
                    { name: 'Update Albums', href: '#updatealbums', icon: <MusicalNoteIcon className="h-6 w-6 text-blue-400" /> },
                    { name: 'Reports', href: '#reportnav', icon: <ChartBarSquareIcon className="h-6 w-6 text-blue-400" /> },
                    { name: 'Profile', href: '/profile', icon: <UserIcon className="h-6 w-6 text-blue-400" /> },
                ].map((item) => (
                    <li key={item.name}>
                        <a
                            href={item.href}
    ...
    >
     ...
     {/* Main Content */}
     <main className="w-5/5 p-6 flex flex-col space-y-6 bg-blue-900 bg-opacity-80">

      {/* Dashboard Header */}
<header className="bg-gray-900 p-4 rounded-lg shadow-md">
  <h1 className="text-3xl font-bold text-gray-300" style={{ fontFamily: 'Poppins, sans-serif' }}>
    Dream Streamer Admin Dashboard
  </h1>
</header>

{/* Top Bar with Search, Notifications, and User Menu */}
<div className="flex justify-between items-center bg-gray-900 p-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out">
  
  {/* Filters */}
  <div className="flex flex-wrap space-x-20" id='homenav'>
...
<div className="bg-gray-900 p-6 rounded-lg shadow-lg" id="mangealbumsnav">
  <h2 className="text-2xl font-bold text-gray-300 mb-6">Manage Albums</h2>
